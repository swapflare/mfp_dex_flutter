...
setState(() {
  loadImage=false;
    ...
    */
    ...
    gecko
        .getTopCoinsMarkets(per_page: 100, category: 'non-fungible-tokens-nft')
        .then((value) {
      if (value != null && value.length > 0) {
        setState(() {
          value.forEach((element) {
            loadedLosersList.add(losers.fromCoinMarketModel(element));
          });
          loadImage = false;
        });
      }
    ...
    return Column(
      children: <Widget>[
        ...
        loadImage
            ...
            : _imageLoaded(context, loadedLosersList),
      ],
    );
  }
}
...
///
///
Widget _imageLoaded(BuildContext context, List<losers> loadedLosersList) {
  return Expanded(
    child: ListView.builder(
      shrinkWrap: true,
      primary: false,
      itemCount: loadedLosersList.length,
      itemBuilder: (ctx, i) {
        return listPriceloser(loadedLosersList[i], ctx);
      },
    ),
  );
}
