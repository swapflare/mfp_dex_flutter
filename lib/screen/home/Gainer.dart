...
class _gainerState extends State<gainer> {
  ...
  ///
  bool loadImage = true;
  List<gainers> loadedGainersList = [];
  ...
  void initState() {
    ...
    */
    ...
    gecko.getTopCoinsMarkets(per_page: 100).then((value) {
      if (value != null && value.length > 0) {
        setState(() {
          value.forEach((element) {
            loadedGainersList.add(gainers.fromCoinMarketModel(element));
          });
          //print('gainer list length:' + loadedGainersList.length.toString());
          loadImage = false;
        });
      }
    });
    ...
    return Column(
      children: <Widget>[
        ...
        loadImage
            ...
            : _imageLoaded(context, loadedGainersList),
      ],
    );
  }
}
...
///
///
Widget _imageLoaded(BuildContext context, List<gainers> loadedGainersList) {
  return Expanded(
    child: ListView.builder(
      shrinkWrap: true,
      primary: false,
      itemCount: loadedGainersList.length,
      itemBuilder: (ctx, i) {
        return listPriceGainers(loadedGainersList[i], ctx);
      },
    ),
  );
}
