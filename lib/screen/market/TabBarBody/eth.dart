...
import 'package:crypto_template/com/ugoc/flutter/coingecko/CoinGeckoClient.dart';
...
class _ethState extends State<eth> {
  ...
  ///
  //var imageNetwork = NetworkImage("https://firebasestorage.googleapis.com/v0/b/beauty-look.appspot.com/o/Artboard%203.png?alt=media&token=dc7f4bf5-8f80-4f38-bb63-87aed9d59b95");
  ...
  @override
  List<ethMarket> loadedethMarketList = [];
  @override
  void initState() {
...
setState(() {
  loadImage=false;
    ...
    gecko.getTopCoinsMarkets(per_page: 100, vs_currency: 'eth').then((value) {
      if (value != null && value.length > 0) {
        setState(() {
          value.forEach((element) {
            loadedethMarketList.add(ethMarket.fromCoinMarketModel(element));
          });
          loadImage = false;
        });
      }
    ...
    return Container(
      ...
      children: <Widget>[
        ...
        loadImage
            ...
            : _dataLoaded(context, loadedethMarketList),
      ],
    ));
  }
}
...
///
///
Widget _dataLoaded(BuildContext context, List<ethMarket> loadedethMarketList) {
  return Expanded(
    child: ListView.builder(
      shrinkWrap: true,
      primary: false,
      itemCount: loadedethMarketList.length,
      itemBuilder: (ctx, i) {
        return card(ctx, loadedethMarketList[i]);
      },
    ),
  );
}
