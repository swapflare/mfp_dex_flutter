...
class _btcState extends State<btc> {
  ...
  @override
  List<btcMarket> loadedbtcMarketList = [];
...
setState(() {
  loadImage=false;
    ...
    gecko.getTopCoinsMarkets(per_page: 100, vs_currency: 'btc').then((value) {
      if (value != null && value.length > 0) {
        setState(() {
          value.forEach((element) {
            loadedbtcMarketList.add(btcMarket.fromCoinMarketModel(element));
          });
        });
      }
    });
    ...
    return Container(
      ...
      children: <Widget>[
        ...
        loadImage
            ...
            : _dataLoaded(context, loadedbtcMarketList),
      ],
    ));
  }
}
...
///
///
Widget _dataLoaded(BuildContext context, List<btcMarket> loadedbtcMarketList) {
  return Expanded(
    child: ListView.builder(
      shrinkWrap: true,
      primary: false,
      itemCount: loadedbtcMarketList.length,
      itemBuilder: (ctx, i) {
        return card(ctx, loadedbtcMarketList[i]);
      },
    ),
  );
}
