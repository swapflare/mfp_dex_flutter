...
class _xrpState extends State<xrp> {
  ...
  List<xrpMarket> loadedxrpMarketList = [];
...
setState(() {
  loadImage=false;
    ...
    gecko.getTopCoinsMarkets(per_page: 100, vs_currency: 'xrp').then((value) {
      if (value != null && value.length > 0) {
        setState(() {
          value.forEach((element) {
            loadedxrpMarketList.add(xrpMarket.fromCoinMarketModel(element));
          });
          loadImage = false;
        });
      }
    ...
    return Container(
      ...
      children: <Widget>[
        ...
        loadImage
            ...
            : _dataLoaded(context, loadedxrpMarketList),
      ],
    ));
  }
}
...
///
///
Widget _dataLoaded(BuildContext context, List<xrpMarket> loadedxrpMarketList) {
  return Expanded(
    child: ListView.builder(
      shrinkWrap: true,
      primary: false,
      itemCount: loadedxrpMarketList.length,
      itemBuilder: (ctx, i) {
        return card(ctx, loadedxrpMarketList[i]);
      },
    ),
  );
}
