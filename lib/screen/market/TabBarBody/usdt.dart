...
class _usdtState extends State<usdt> {
  ...
  List<usdtMarket> loadedusdtMarketList = [];
...
setState(() {
  loadImage=false;
    ...
    gecko.getTopCoinsMarkets(per_page: 100, vs_currency: 'usdt').then((value) {
      if (value != null && value.length > 0) {
        setState(() {
          value.forEach((element) {
            loadedusdtMarketList.add(usdtMarket.fromCoinMarketModel(element));
          });
        });
      }
    });
    ...
    return Container(
      ...
      children: <Widget>[
        ...
        loadImage
            ...
            : _dataLoaded(context, loadedusdtMarketList),
      ],
    ));
  }
}
...
///
Widget _dataLoaded(
    BuildContext context, List<usdtMarket> loadedusdtMarketList) {
  return Container(
    child: ListView.builder(
      shrinkWrap: true,
      primary: false,
      itemCount: loadedusdtMarketList.length,
      itemBuilder: (ctx, i) {
        return card(ctx, loadedusdtMarketList[i]);
      },
    ),
  );
}
